/**
 * 主播介绍页（定稿首页）：主播卡（头像/签名/平台事实徽标）→ 复盘卡（首卡，
 * 裁决三）→ 制片人简报 → 运行概览（薄统计带 + 花费公示）→ 舰长栏。
 * 取代旧 Dashboard；宏观态势直呈段整段退役（简报即态势展示面）。
 */
...
/** 「分层跑」次级菜单：ai_audience 从平铺位移入本菜单，引导文案原样保留
...
export function Streamer({ roomId }: { roomId: string }) {
  ...
  // 主钮旁预估行——/api/budget 的 estimate 段（RunTracker 终态失效会连它一起刷新）。
  ...
  if (overview.isError) {
    ...
    // 空态判别走 ApiError.status（404 = 尚无采集快照），不再子串匹配文案。
  ...
  return (
    <>
      ...
      <section className="section card">
        ...
        {cost !== null && (
          // 价目内联公示，不再指引跳源码（constants.ts TOKEN_RATES 同值真源）。
      ...
      {/* 「态势项」胶囊与宏观折叠组整段退役（裁决）——简报（front_brief）
