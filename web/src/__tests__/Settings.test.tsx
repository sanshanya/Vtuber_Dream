 ...
 * - 单次预算行（/api/budget budget_cny）：null 预算 → 「未设」。
 */
...
const BUDGET = {
  budget_cny: 4.0,
  estimate: { roster_viewers: 3, fresh_viewers: 3, estimated_cny: 6.0, etd_minutes: [4, 6] },
};
...
describe("Settings 白名单五键（Z6 件5）", () => {
  ...
  it("单次预算行渲染 budget_cny；null（未设闸）→ 「未设」", async () => {
    stubSettingsFetch(CONFIG, BUDGET);
    renderSettings();
    await waitFor(() => expect(screen.getByTestId("budget-note")).toBeTruthy());
    expect(screen.getByTestId("budget-note").textContent).toContain("¥4.00");
    cleanup();

    stubSettingsFetch(
      { ...CONFIG, ai: { ...CONFIG.ai, run_budget_cny: null } },
      { budget_cny: null, estimate: null },
    );
    renderSettings();
    await waitFor(() => expect(screen.getByTestId("budget-note")).toBeTruthy());
    expect(screen.getByTestId("budget-note").textContent).toContain("未设");
  });
});