...
fn python_order<T: serde::Serialize>(submission: &T, order: &[&str]) -> Value {
    ...
    // Python 字面不含、有意剥落）与 front_brief（合成 demo 有意让简报缺席，
...
/// `demo.py build_demo`（裁剪 peers/HTML 后）+ 默认输出根 = `output_dir 同级/_demo`。
///
/// 返回值 = Python build_demo dict 的裁剪版（无 report 键）：
        ...
        let overall = AudienceSituationSubmission {
            executive_summary:
                "合成Demo显示：两名观众通过不同内容角度连接到《异环》，另一名观众形成Vocaloid与Blender创作型独立兴趣。"
                    .to_string(),
            // 合成 demo 同样让 front_brief 缺席——BriefingCard 空缺位即演示面。
