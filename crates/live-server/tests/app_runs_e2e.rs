...
async fn budget_gate_blocks_then_briefing_only_passes_to_done() {
    ...
    .unwrap();
    // 布景②：预算注入——yaml_template 无 run_budget_cny 行（line-level 重写没锚，故直接嵌）。
    let yaml = std::fs::read_to_string(&config_path).unwrap();
    assert!(yaml.contains("  api: chat_completions\n"), "{yaml}");
    let pinned = yaml.replace(
        "  api: chat_completions",
        "  api: chat_completions\n  run_budget_cny: \"0.01\"",
    );
    std::fs::write(&config_path, pinned).unwrap();

    // ── 相位一：0.01 预算 + full → 阻断 failed ──
    ...
    assert_eq!(snapshot["status"], "done", "{snapshot}");
    assert!(
        snapshot["outcome"].get("budget_block").is_none(),
        "放行相位不得带 budget_block：{snapshot}"
    );
    let events: Vec<&str> = snapshot["events"]
        .as_array()
        .unwrap()
