...
/// 状态序（design §10 枚举字面）。
/// WS 弹幕窗已解耦为独立入口（live-audience ws-record / ws_record::run_ws_record）——
/// 不再占 run 状态机一相（12h 保险丝不再劫持单 run 槽）。
                ...
                let outcome: Result<Value, PipelineError> = (move || {
                    ...
                    if !ai_only {
                        ...
                        if collect_only {
                            ...
                            //（WS 弹幕窗不再随 collect 尾段开——独立入口
                            // live-audience ws-record / ws_record::run_ws_record。）
